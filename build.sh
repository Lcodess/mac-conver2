...
then
	...
	dotnet publish \autodraw.csproj -r osx-arm64 -c Release -p:publishsinglefile=true --self-contained true -p:debugsymbols=false -p:debugtype=none -o Builds/Autodraw-macos-arm64-$buildExtension/Autodraw
	cd Builds/Autodraw-macos-arm64-$buildExtension
	zip -r $OLDPWD/Builds/Autodraw-macos-arm64-$buildExtension.zip .
	cd -
	rm -r Builds/Autodraw-macos-arm64-$buildExtension
fi
